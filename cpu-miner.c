...
int main(int argc, char *argv[])
{
	...
	applog(LOG_INFO, "Shaing %s once",argv[1]);

	unsigned char *p= malloc(strlen(argv[1])/2);
	applog(LOG_INFO,"Allocated %d bytes",strlen(argv[1])/2);
	hex2bin(p,argv[1], strlen(argv[1]));
	unsigned char *hash1=malloc(129);
	sha256d(hash1,p, strlen(argv[1])/2);
	char* out= malloc(129);
	bin2hex(out,hash1,129);
	applog(LOG_INFO,"Output: %s",out);
